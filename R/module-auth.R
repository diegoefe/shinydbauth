          ...
          tags$div(
            ...
            tags$div(
              ...
              tags$div(
                ...
                tags$h3(lan$get("Please authenticate"), id = ns("shinydbauth-auth-head"))
              ),
              tags$br(),
              textInput(
                inputId = ns("user_id"),
              ...
              uiOutput(ns("update_shinydbauth_language"))
            )
          )
        )
      )
  ...
  observe({
    if(!is.null(input$language)){
      ...
      session$sendCustomMessage(
        ...
        message = list(
          inputId = ns("shinydbauth-auth-head"),
          title = lan()$get("Please authenticate")
        )
      )

      output$update_shinydbauth_language <- renderUI({
  ...
  observeEvent(input$go_auth, {
    ...
    if (isTRUE(res_auth$result)) {
      ...
      if (isTRUE(use_token)) {
        ...
        .tok$set_user(token, input$user_id)
