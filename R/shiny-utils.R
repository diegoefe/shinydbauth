...
shinydbauth_where <- function(where) {
  # tags$script(sprintf(
  #   paste0("$(document).ready(function(){Shiny.setInputValue('shinydbauth_where', '", where, "');})")
  # ))
  # humm, little hack ^^
  tags$div(
    style = "display: none;",
    selectInput(inputId = "shinydbauth_where", label = NULL, 
               choices = where, selected = where, multiple = FALSE)
  )
}

shinydbauth_language <- function(lan) {
  tags$div(
    style = "display: none;",
    selectInput(inputId = "shinydbauth_language", label = NULL, 
